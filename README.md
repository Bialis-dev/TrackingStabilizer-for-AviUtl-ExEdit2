...
が出力した追跡点(図形オブジェクト)の座標を読み取り、基準フレームからの移動量を打ち消す方向に映像を動かすことで、
手ブレを補正します。
...
3. 「追跡レイヤー」に手順1で図形を挿入したレイヤー番号を指定する
4. 「基準フレーム」にブレの無い基準フレームを指定する
5. プレビューで確認しながら、補正強度・平滑化を調整する

## 制限事項

- 平行移動(X/Y)のみの補正です。回転・拡大縮小によるブレには対応していません
