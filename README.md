...
- 平行移動(X/Y)に加えて、回転(ロール)ブレの補正にも対応(任意)
...
5. (任意)回転ブレも補正したい場合は、別の特徴点をもう1つ追跡し、2点を結ぶ直線の角度のズレを同様に打ち消す
...
| 項目 | 説明 |
|---|---|
| 回転を補正する | ONにすると、追跡レイヤー1・2の2点を使って回転(ロール)ブレも補正する |
| 追跡レイヤー2 | 回転補正用の2点目の図形オブジェクトがあるレイヤー番号 |
| 回転補正強度(%) | 100%で回転ズレをそのまま打ち消す |
| 回転方向を反転する | 補正が逆向きに回ってしまう場合にON |

回転補正には、追跡レイヤー1とは別に**2点目のトラッキング**が必要です。2点を
結ぶ直線の角度が基準フレームからどれだけズレたかを毎フレーム計算し、その分を
打ち消す方向に回転させます。1点だけの追跡では「回転した」のか「平行移動した」
のか区別できないため、回転補正には必ず2点の追跡が必要になります。
...
6. (任意)回転ブレも補正したい場合、手順1と同様にもう1つ別の特徴点を追跡して
   図形を挿入し、「回転を補正する」をON、「追跡レイヤー2」にそのレイヤー番号を
   指定する。回転が逆向きにかかる場合は「回転方向を反転する」をONにする

## 制限事項

- 拡大縮小(カメラの前後の揺れ)によるブレには対応していません
